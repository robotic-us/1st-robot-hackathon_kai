...
class PersistentPcmSession:
    ...
    def stop(self, on_event: Optional[EventCallback] = None) -> dict[str, object]:
        ...
        return stop_motion_and_hold(self._client, on_event=on_event)
    ...
    def run(
        ...
        *,
        ...
        for repeat_index in range(1, job.repeat + 1):
            if repeat_index > 1:
                raise RuntimeError(
                    "automatic repeat blocked while previous pose is servo-held"
                )
            if cancel_requested():
...
class PcmSessionManager:
    ...
    def _on_event(self, event: str, value: object | None) -> None:
        state_for_event = {
            ...
            "safety_stop_and_hold": SessionState.ARMED,
