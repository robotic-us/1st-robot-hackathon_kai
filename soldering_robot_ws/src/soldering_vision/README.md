...
/camera/image_raw
        ...
        ▼
YOLO segment/pose
  ├─ red_wire   → moving_wire
  ├─ black_wire → fixed_wire
  └─ iron       → iron_tip
        │
...
YOLO는 빨간 전선·검은 전선·인두의 존재, mask, 위치와 interaction crop을
담당한다. ConvNeXt는 crop에서 두 전선의 접촉과 인두-접합부 접촉이
모두 충족되었는지를 이진 분류한다. `PREHEAT`, `FEED` 같은 공정 단계는
이미지 분류 클래스에 넣지 않고 상위 supervisor가 판단한다. 한 대의
카메라에서 임의의 깊이를 추정하지 않으며,
고정 지그의 작업 평면에 대해서만 homography로 mm 좌표를 만든다.

`GeometryObservation.valid`는 다음 조건을 모두 만족할 때만 참이다.

- 카메라/homography 보정 완료
- red wire, black wire, iron tip이 신뢰도 기준 이상
- ConvNeXt confidence가 기준 이상
...
원본 프레임을 다음 구조로 정리한다. `solderable`은 두 전선과
인두가 접합부에 모두 접촉한 상태이며, 조건 하나라도 빠지면
`not_solderable`이다. session 이름이 train/val 분할 단위다.
