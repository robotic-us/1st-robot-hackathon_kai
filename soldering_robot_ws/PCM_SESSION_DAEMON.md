...
SDO를 통한 최선형 소프트웨어 정지이며 **서보 OFF를 보내지 않는다**. 부하를
잡는 토크는 계속 유지한다. 물리 비상정지 장치를 대체하지 않는다.
...
`default_axes`를 사용한다. 현재 PCM의 영점 저장은 서보 OFF를 요구하므로,
중력 부하가 있는 실제 기구에서 `repeat>1` 상대교시는 첫 동작 뒤 HOLDING으로
멈춘다. 기계적 지지 없이 자동 서보 OFF를 허용하지 않는다.
...
- 데몬 종료 시 모션 정지를 요청하고 서보 토크는 유지한 뒤 포트를 닫는다.
