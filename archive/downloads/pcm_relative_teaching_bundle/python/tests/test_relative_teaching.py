...
def test_rebase_arm_second_stage_has_explicit_call_order(monkeypatch):
    calls = []
    ...
    def parked(actual_client, **_kwargs):
        calls.append(("parked", actual_client))
        return {"servo_state": 0, "motion_busy": False}
    ...
    monkeypatch.setattr(teaching, "stop_and_servo_off", cleanup)
    monkeypatch.setattr(teaching, "wait_for_rebase_window", parked)
    ...
    assert result == {
        ...
        "parked": {"servo_state": 0, "motion_busy": False},
    }
...
def test_idle_cleanup_does_not_send_motion_zero(monkeypatch):
    client = _FakeClient()
    monkeypatch.setattr(
        teaching,
        "read_status",
    ...
    result = teaching.stop_and_servo_off(client)
...
def test_relative_slot_batch_reuses_one_live_session(monkeypatch):
    ...
    result = teaching.run_relative_slots_from_current(
