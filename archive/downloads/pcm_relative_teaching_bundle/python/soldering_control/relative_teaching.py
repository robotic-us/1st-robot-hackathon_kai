    ...
    def check_cancel() -> None:
        if cancel_requested is not None and cancel_requested():
            ...
            try:
                ...
                raise MotionCancelled(f"{label}: cancellation requested")
...
def stop_and_servo_off(
    ...
    *,
    ...
    """Best-effort hardware cleanup; never hides the primary motion result."""
    errors: list[str] = []
    ...
    result = {"attempted": True, "status_before": status, "errors": errors}
...
def run_first_slot_stage(
    ...
    *,
    ...
    """Arm if necessary, execute the first slot, then request servo off."""
    status = read_status(client)
    ...
    cleanup = stop_and_servo_off(client, on_event=on_event)
    parked = wait_for_rebase_window(client, timeout=12.0)
    _emit(on_event, "servo_parked", parked)
    return {
        "arm": arm,
        "slot": first,
        "cleanup": cleanup,
    ...
    *,
    ...
    with factory() as first_client:
        ...
        try:
            ...
            stop_and_servo_off(first_client, on_event=on_event)
            raise
    ...
    with factory() as second_client:
        ...
        try:
            ...
            stop_and_servo_off(second_client, on_event=on_event)
            raise
...
def run_relative_slots_from_current(
    ...
    *,
    ...
    still performs origin -> arm -> slot -> servo-off, but avoiding a reset
